  ...
  public analyze(context: SpecialistContext): SpecialistAnalysis {
    ...
    return {
      ...
      observations: [
        ...
        hasEnoughRefs ? 'Suporte mínimo de 5 referências atendido.' : 'Aviso: Abaixo do mínimo de referências.'
      ],
      ...
      recommendations: [
        {
          ...
          type: 'MONITOR',
          title: 'Monitorização Contínua de Mercado',
...
export class MediaStrategistAgent extends BaseSpecialist {
  ...
  public analyze(context: SpecialistContext): SpecialistAnalysis {
    ...
    return {
      ...
      observations: [
        hasBudget ? `Orçamento diário máximo configurado: R$ ${context.budgetConstraints?.dailyMax}` : 'Orçamento diário não especificado.'
      ],
      evidence: ['Meta Ads apresenta melhor retorno histórico para o objetivo de conversão.'],
      hypotheses: [
        {
          id: `hyp_med_${Date.now()}`,
          statement: 'Distribuição orçamentária 70/30 (Aquisição / Remarketing) otimiza o funil.',
          specialist: 'MEDIA_STRATEGIST',
          supportingEvidence: ['Equilíbrio entre captação e remarketing.'],
          contradictingEvidence: [],
          confidence: confidence.confidenceScore,
          status: 'SUPPORTED'
        }
...
export class PerformanceAnalystAgent extends BaseSpecialist {
  ...
  public analyze(context: SpecialistContext): SpecialistAnalysis {
    ...
    return {
      ...
      observations: [
        hasMetrics ? `Métricas históricas detetadas (ROAS: ${metrics.roas || 'N/A'}).` : 'Sem métricas históricas.'
      ],
...
export class OfferStrategistAgent extends BaseSpecialist {
  ...
  public analyze(context: SpecialistContext): SpecialistAnalysis {
    ...
    return {
      ...
      observations: [
        hasPrice ? `CURRENT_PRICE informado: R$ ${context.currentPrice}` : 'Preço atual não informado.',
        `Margem alvo: ${context.margin || 'N/A'}%`
      ],
      evidence: ['Cálculo de margem de contribuição e ponto de equilíbrio.'],
      hypotheses: [
        {
          id: `hyp_off_${Date.now()}`,
          statement: 'Precificação alinhada à margem alvo garante sustentabilidade do CAC.',
          specialist: 'OFFER_STRATEGIST',
          supportingEvidence: ['Relação direta entre ticket, margem e teto de CAC.'],
          contradictingEvidence: [],
          confidence: confidence.confidenceScore,
      ...
      recommendations: [
        {
          ...
          type: 'PRICE',
          title: 'Manter Alinhamento entre Preço e Margem',
          description: 'Garantir que o custo de aquisição permaneça abaixo do limite de lucro.',
          specialist: 'OFFER_STRATEGIST',
          priority: 'HIGH',
...
export class CreativeStrategistAgent extends BaseSpecialist {
  ...
  public analyze(context: SpecialistContext): SpecialistAnalysis {
    ...
    return {
      ...
      observations: [
        'Direcionamento de produção criativa: Formatos 9:16.',
        `Produto base: ${context.productOrService || 'Não especificado'}`
      ],
      evidence: ['Formatos verticais com hooks direcionados geram melhor retenção.'],
      hypotheses: [
        {
          id: `hyp_cre_${Date.now()}`,
          statement: 'Hooks de interrogação elevam CTR.',
          specialist: 'CREATIVE_STRATEGIST',
          supportingEvidence: ['Padrões de consumo móvel.'],
          contradictingEvidence: [],
          confidence: confidence.confidenceScore,
          status: 'SUPPORTED'
        }
